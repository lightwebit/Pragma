...
import { ref } from 'vue'
import type { PragmaAnalysis } from '../stores/session'

defineProps<{ data: PragmaAnalysis; collapsed: boolean }>()
defineEmits<{ toggle: [] }>()
...
<template>
  <div class="pragma-card analysis-card">
    <div class="card-header" @click.self="$emit('toggle')">
      ...
      <span class="card-label" @click="$emit('toggle')">ANALYSIS</span>
...
.card-header:hover { background: rgba(74, 144, 217, 0.14); }
