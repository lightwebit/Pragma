...
import { ref } from 'vue'
import type { PragmaRawNote } from '../stores/session'

defineProps<{ data: PragmaRawNote; collapsed: boolean }>()
defineEmits<{ toggle: [] }>()
...
<template>
  <div class="pragma-card raw-note-card">
    <div class="card-header" @click.self="$emit('toggle')">
      ...
      <span class="card-label" @click="$emit('toggle')">MODEL OUTPUT</span>
      <span class="unstructured-badge">unstructured</span>
...
.card-header:hover { background: rgba(128, 128, 128, 0.12); }
...
.card-collapse.is-collapsed { grid-template-rows: 0fr; }
